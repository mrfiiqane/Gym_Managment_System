...
<main class="space-y-4 animate-fadeIn  dark:bg-darkPanel w-87.5 md:w-full rounded-3xl shadow-lg border border-primary/5">
  ...
  <header class="flex items-center justify-between p-2 backdrop-blur-md border-b border-primary/5">
    <div class="flex items-center gap-4">
      <h1 class="text-3xl font-black text-slate-800 dark:text-white tracking-tight">All system_link Management </h1>
    </div>
  </header>
  <div class="bg-white dark:bg-darkPanel rounded-3xl shadow-lg border border-primary/5 overflow-hidden">
    <div class="overflow-x-auto">
      <div class="p-4 border-b border-primary/5 flex justify-between items-center">
        <div class="flex items-center gap-2">
          ...
          <p class="text-sm font-medium text-slate-500 dark:text-slate-400">
            Total system_links: <span id="totalCount" class="font-bold text-slate-800 dark:text-white">0</span>
          </p>
        </div>
        <button onclick="openModal()"
          class="bg-primary text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-primary/90 transition shadow-lg shadow-primary/20">+
          Add system_links</button>
      </div>
      ...
      <div class="m-2 p-2 w-full max-w-sm md:block relative">
        ...
        <input type="text" id="linksSearch" placeholder="Search by name or contact..."
      ...
      <div class="overflow-x-auto">
        <table id="links_table" class="w-full text-left">
    ...
    <div
      class="p-6 border-t border-slate-50 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-white/2">
      <p id="paginationInfo" class="text-xs font-medium text-slate-500 dark:text-slate-400">Showing 0 of 0 system_links</p>
      <div class="flex items-center gap-2">
        <button id="prevPage"
          class="p-2 rounded-lg hover:bg-white dark:hover:bg-white/10 shadow-sm border border-transparent hover:border-primary/10 transition-all disabled:opacity-30">
...
<div id="modal" class="fixed inset-0 hidden items-center justify-center z-50 backdrop-blur-sm transition-all">
  <div
    class="bg-white dark:bg-darkPanel p-8 rounded-[2rem] h-8/10 max-w-[580px] relative shadow-2xl border border-primary/10 m-4">

    <div class="flex justify-between items-center mb-8">
      <div>
        <h3 id="modalTitle" class="text-2xl font-black text-slate-800 dark:text-white">Add New links</h3>
        <p class="text-xs text-slate-400 font-medium mt-1 uppercase tracking-widest">links Information System</p>
      </div>
    ...
    <form id="links_form" class="space-y-4" method="post">
      <div id="success"
      ...
      <div id="error" class="hidden p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-100 border border-red-200 shadow-lg"
        role="alert">this is errors message </div>
      <input type="hidden" name="id" id="update_id">

      <div class="space-y-1.5">
        <label
          class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">Name</label>
      ...
      <div class="space-y-1.5">
        <label
          class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">Link</label>
      ...
      <div class="space-y-1.5">
        <label
          class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">Category</label>
      ...
      <div class="flex justify-end gap-3 pt-6">
        <button type="button" onclick="closeModal()"
          class="px-6 py-3 text-slate-500 font-bold text-sm hover:bg-slate-100 dark:hover:bg-white/5 rounded-2xl transition-all">Cancel</button>
        <button type="submit" id="btnSave"
          class="px-10 py-3 bg-primary text-white font-bold rounded-2xl hover:bg-primary/90 shadow-xl shadow-primary/20 transition-all text-sm">Save
          links</button>
      </div>
    </form>
  </div>
...
<script>
  function openModal() {
    ...
    $("#update_id").val("");
    $("#action").val("Insert");
    $("#modalTitle").text("Add New links");
    $("#btnSave").text("Save links");
