    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function closeAllSubMenus() {
            Array.from(sidebar.getElementsByClassName('submenu')).forEach(ul => {
                ...
                ul.previousElementSibling.querySelector('.chevron')?.classList.remove('rotate-180');
            })
        }

        // Sidebar Profile Image Upload Logic
        ...
        if (fileImage && showInput) {
            fileImage.addEventListener("change", function(e) {
                ...
                $.ajax({
                    method: "POST",
                    dataType: "JSON",
                    url: "<?php echo BASE_URL; ?>api/sidebar.php",
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(data) {
                        if(data.status) {
                            if(typeof Toast !== 'undefined') Toast.show(true, "Profile image updated successfully!");
                        } else {
                            if(typeof Toast !== 'undefined') Toast.show(false, data.message || "Failed to update image");
                        }
                    },
                    error: function() {
                        if(typeof Toast !== 'undefined') Toast.show(false, "An error occurred");
                    }
                });
            });
        }
